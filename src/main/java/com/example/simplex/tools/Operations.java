...
public class Operations {
    ...
    private static GaussTable.Solution type = GaussTable.Solution.ONE;
    private static List<Integer> basisList;

    public static GaussTable gauss(List<Integer> basisList, Fraction[][] matr) {
        type = GaussTable.Solution.ONE;
        GaussTable gaussResult = new GaussTable();
        basisList = basisList.stream().sorted().collect(Collectors.toList());
        Operations.basisList = new ArrayList<>(basisList);
    ...
    public static Fraction stringToFraction(String value) throws IllegalArgumentException {
        ...
        throw new IllegalArgumentException("Строка не может быть преобразована к дроби");
    }
